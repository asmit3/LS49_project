...
def strong_spot_mask(refls, panel_size, n_panels):
    ...
    for pid in range(n_panels):
      refl_tbl = refls.select(refls['panel']==pid)
      if len(refl_tbl)==0:
          spot_mask = np.zeros(panel_size, bool)
          all_spot_masks.append(spot_mask) 
          continue
      Nrefl = len(refl_tbl)
    ...
    return np.array(all_spot_masks)
...
def process_ls49_image_real(tstamp='20180501143555114', #tstamp='20180501143559313',
    ...
    GAIN = 1.0
    loader = dxtbx.load(os.path.join(ls49_data_dir,'../jungfrau_cbf','jungfrauhit_%s.cbf'%tstamp))
    cbf_imageset = loader.get_imageset([os.path.join(ls49_data_dir,'../jungfrau_cbf','jungfrauhit_%s.cbf'%tstamp)])
    ...
    R2 = flex.reflection_table.from_file(os.path.join(ls49_data_dir, 'idx-jungfrauhit_%s_strong.refl'%tstamp))
    n_panels=16
    ...
    chann_lambda, channI = np.array(pickle.load(open(os.path.join(ls49_data_dir,'../rayonix_images_4_or_more_spots_r183_255',fee_file), 'r'))[tstamp]).T
