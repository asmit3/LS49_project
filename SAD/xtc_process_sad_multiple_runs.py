...
message = ''' Run xtc_process_sad by specifying multiple runs at once. Can optimize resource usage on Cori in this manner. Had to copy over the run method of InMemScript and put all the multiple runs specific code there. '''
...
phil_scope = parse(LS49_phil_str + xtc_phil_str + dials_phil_str + extra_dials_phil_str + db_phil_str + radial_average_phil_str, process_includes=True).fetch(parse(program_defaults_phil_str))
# Function imports
...
class InMemScript_LS49_SAD_multiple_runs(InMemScript_LS49_SAD):
...
if __name__ == "__main__":
  ...
  try:
    script = InMemScript_LS49_SAD_multiple_runs()
    script.run()
  except Exception as e:
    halraiser(e)
