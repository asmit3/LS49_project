...
class FEE_spec(object):
  ...
  def vignetting_correction_h5(self, plot=False):
    ...
    if plot:
      ...
      plt.figure()
    for run in runs:
      with h5py.File('/global/cscratch1/sd/asmit/LS49/LS49_SAD_v3/hdf5/smalldata/ana-run{:03d}.h5'.format(run)) as fid:
    ...
    if plot:
      plt.plot(list(xs),self.vignetting_factor)
      plt.show()
  ...
  def beam_profile_correction(self, 
    ...
    if True:
      ...
      for i_energy, central_energy in enumerate(central_energies):
        ...
        if plot:
          #plt.plot(x,y)  # Not plotting raw data for paper
          plt.xlabel('Energy dispersion axis')
          plt.ylabel('FEE spectrometer intensity')
          plt.plot(x,gaus(x,*popt))
          # Below stuff only for paper
          tt=(x,y, gaus(x,*popt))
          all_tt.append(tt)
      ...
      dump('beam_profile_correction_1.pickle', all_tt)
      #exit()
      ...
      #plt.plot(x,beam_profile_cs(x),'*-',c='blue')
      ...
      if True:
       ...
       # Below stuff only for paper
       #tt=(xs,ys)
       #dump('beam_profile_correction_2.pickle', tt)
       #from IPython import embed; embed(); exit()
       plt.plot(xs[500:1500],ys[500:1500],c='b')
...
#
  ...
  def calibrate_fee_spec(self, plot=False):
    ...
    if plot:
      ...
      #from libtbx.easy_pickle import dump
      #tt=(x,y)
      #dump('calibration_plot.pickle', tt)
      plt.show()
  ...
  def analyze_photon_energy(self, plot=False):
    ...
    # all_scaled_data for paper dumping
    all_scaled_data = []
    for nevent,evt in enumerate(ds.events()):
  ...
  def get_Fe_edge(self, plot=False):
    ...
    if plot:
      ...
      from libtbx.easy_pickle import dump
      #dump('R143_Fe.pickle', fee_data)
      #exit()
      plt.plot(fee_data[:])
      plt.xlabel('pixels on spectrometer', fontsize=20)
      plt.ylabel('Intensity recorded on spectrometer', fontsize=20)
...
if __name__ == '__main__':
  message =        """export SIT_DATA=/global/cscratch1/sd/asmit/LS10/psdm/psdm/data
                   export SIT_PSDM_DATA=/global/cscratch1/sd/asmit/LS49/LS49_SAD_v3"""
  short_circuit=True
  dump_pickle=True # False
  if short_circuit and dump_pickle :
    FEE = FEE_spec(run=222,Fe_foil_run=143)
    FEE.get_Fe_edge(plot=False)
    FEE.vignetting_correction_h5(plot=True)
    FEE.beam_profile_correction(plot=True)
    #from IPython import embed; embed(); exit()
  ...
  if not dump_pickle:
    ...
    FEE.run=222
    #from IPython import embed; embed(); exit()
    FEE.analyze_photon_energy(plot=True)
